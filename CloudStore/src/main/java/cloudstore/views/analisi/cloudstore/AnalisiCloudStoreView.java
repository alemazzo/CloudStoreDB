...
public class AnalisiCloudStoreView extends AbstractJavaFXView {
  ...
  @Override
  public void init() {
    final TableColumn<CloudStoreAnalysisOperation, Integer> codiceColumn = new TableColumn<>("Codice");
    codiceColumn.setCellValueFactory(u -> new SimpleIntegerProperty(u.getValue().codice).asObject());

    final TableColumn<CloudStoreAnalysisOperation, String> operationColumn = new TableColumn<>("Operazione");
    ...
    this.operationTable.setItems(
        new ObservableListWrapper<>(Arrays.stream(CloudStoreAnalysisOperation.values()).collect(Collectors.toList())));

    this.operationTable.getSelectionModel().getSelectedItems().addListener((ListChangeListener<? super CloudStoreAnalysisOperation>) (selected) -> {
      final CloudStoreAnalysisOperation operation = selected.getList().stream().findFirst().get();
      try {
        final Set<QueryObjectResult> results = this.getAnalisiController().getOperationResult(operation);
